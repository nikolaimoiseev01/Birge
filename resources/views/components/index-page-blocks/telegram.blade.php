<section id="platform" class=" birge-bg  pt-[90px] pb-[22px] text-white sm:px-4 relative">
    <div class="mx-auto container">
        ...
        >
            ...
            <x-Ui.link type="mint" class="!text-azure-500 fill-azure-500 z-40 relative">
                <x-slot:icon>
        ...
        <div class=" border-b border-bright-40/40 grid grid-cols-3 gap-3 lg:grid-cols-2 md:grid-cols-1  pb-[180px]">
            @foreach($telegramPosts as $i => $post)
                <article
                    x-data="revealOnScroll({{ $i * 70 }})"
                    :class="shown ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'"
                    class=" border-gradient min-h-[120px] flex flex-col relative z-40 justify-between rounded-lg  bg-white/5 p-5 transition-all duration-700 ease-out hover:bg-white/10"
                >
